...
Else
    ...
    "where Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset & " AND (Transaksi='AVD' or Transaksi='VOD' or Transaksi='CRC')")
...
lbShiftReset.Caption = "Shift"
Else
  lbShiftReset.Caption = "Reset X atau Z (1=X atau 2=Z)                Cetak <ENTER>, Keluar <ESC>"
End If
...
Set dbc = Nothing
End Sub
Function IsAdaTransaksiPending() As Boolean
Dim dbc As Database
Dim rsc As Recordset

Set dbc = OpenDatabase(DIRPOS & "\Database\TempDB" & Format(Now, "_yyyyMM") & ".mdb")
Set rsc = dbc.OpenRecordset("SELECT MSales.NoID FROM MSales " & _
    "WHERE IsPending=-1 and Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset)
If rsc.EOF And rsc.BOF Then
  IsAdaTransaksiPending = False
Else
  IsAdaTransaksiPending = True
End If
rsc.Close
Set rsc = Nothing
dbc.Close
...
Dim rs As Recordset
    '========MReset
      ...
      strqry = "SELECT SUM(Subtotal) AS Subtotal1, SUM(DiscIntern) AS DiscNota1, SUM(Pembulatan) AS Pembulatan1, SUM(Subtotal-DiscIntern-Pembulatan) AS Total1, SUM(Voucher) AS Voucher1," & vbCrLf & _
               " SUM(HargaTotal-Bank-Voucher) AS Tunai1, SUM(ReedemPoin) AS RedeemPoin1, SUM(NilaiReedemPoin) AS NilaiReedemPoin1, " & vbCrLf & _
               ...
               " (SELECT SUM(BANK) FROM MSales WHERE IDJenisKartu=1 AND Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset & ") AS CreditVisa, " & vbCrLf & _
               ...
               " (SELECT SUM(BANK) FROM MSales WHERE IDJenisKartu=2 AND Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset & ") AS CreditMaster, " & vbCrLf & _
               ...
               " (SELECT SUM(BANK) FROM MSales WHERE IDJenisKartu=4 AND Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset & ") AS CreditBCA, " & vbCrLf & _
               ...
               " (SELECT SUM(BANK) FROM MSales WHERE IDJenisKartu=7 AND Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND Shift= " & NamaShiftReset & ") AS CreditLain, " & vbCrLf & _
               ...
               " (SELECT SUM(MSalesD.DiscInternRp*MSalesD.Qty) FROM MSales INNER JOIN MSalesD ON MSalesD.IDSales=MSales.NoID WHERE MSales.IDMember>=1 AND Day(MSales.Tanggal)=" & DayReset & " AND Month(MSales.Tanggal)=" & MonthReset & " AND Year(MSales.Tanggal)=" & YearReset & " AND MSales.Shift= " & NamaShiftReset & ") AS DiscMember2 " & vbCrLf & _
      ...
      If Not (rs.EOF And rs.BOF) Then
        ...
        If Text1.Text = 2 Then
          If Not NullToBool(RsReset!ResetZ) Then
            ...
            RsReset!DiscMember2 = NullToNol(rs!DiscMember2)
            ...
            RsReset!ReedemPoin = NullToNol(rs!RedeemPoin1)
            RsReset!NilaiReedemPoin = NullToNol(rs!NilaiReedemPoin1)
    ...
    '==============
End Sub
Private Sub ResetPerShift()
Dim dbs As Database

Dim RsReset As Recordset
Dim strqry As String
Dim rs As Recordset
Dim rsRetur As Recordset
Dim RsKasKeluar As Recordset
Dim NoIDReset As Long
Dim KodeKasir_ As String
Dim NamaKasir_ As String
    '========MReset
      Set dbs = OpenDatabase(DIRPOS & "\database\TempDB" & Format(Now, "_yyyyMM") & ".mdb")
      Set RsReset = dbs.OpenRecordset("SELECT * FROM MReset WHERE Day(Tanggal)=" & DayReset & " AND MONTH(Tanggal)=" & MonthReset & " AND YEAR(Tanggal)=" & YearReset & " AND Shift=" & NamaShiftReset)
'      strqry = "SELECT MSales.IDUser, MSales.Shift, SUM(Subtotal) AS Subtotal1, SUM(DiscIntern) AS DiscNota1, SUM(Pembulatan) AS Pembulatan1, SUM(Subtotal-DiscIntern-Pembulatan) AS Total1, SUM(Voucher) AS Voucher1," & vbCrLf & _
'               " SUM(HargaTotal-Bank-Voucher) AS Tunai1, " & vbCrLf & _
'               " SUM(iif(IDBANK=1,Bank,0)) AS Debet, " & vbCrLf & _
'               " SUM(iif(IDBank=2,MSales.BANK-(MSales.HargaTotal-(MSales.SubTotal-MSales.DiscIntern-MSales.Pembulatan)),0)) AS Kredit, " & vbCrLf & _
'               " SUM(MSales.HargaTotal-(MSales.SubTotal-MSales.DiscIntern-MSales.Pembulatan)) AS Charge1, " & vbCrLf & _
'               " COUNT(NoID) AS JumNota, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=5,BANK,0)) AS DebetMandiri, " & vbCrLf & _
'               " Sum(iif(IDJenisKartu=5,1,0)) AS CountDebetMandiri, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=3,BANK,0)) AS DebetBCA, " & vbCrLf & _
'               " Sum(iif(IDJenisKartu=3,1,0)) AS CountDebetBCA, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=6,BANK,0)) AS DebetLain, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=6,1,0)) AS CountDebetLain, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=1,BANK,0)) AS CreditVisa, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=1,1,0)) AS CountCreditVisa, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=2,BANK,0)) AS CreditMaster, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=2,1,0)) AS CountCreditMaster, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=4,BANK,0))  AS CreditBCA, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=4,1,0)) AS CountCreditBCA, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=7,BANK,0))  AS CreditLain, " & vbCrLf & _
'               " SUM(iif(IDJenisKartu=7,1,0))  AS CountCreditLain, " & vbCrLf & _
'               " (SELECT SUM(MSalesD.Jumlah) FROM MSales INNER JOIN MSalesD ON MSalesD.IDSales=MSales.NoID WHERE UCASE(MSalesD.Transaksi)='RTN' AND Day(MSales.Tanggal)=" & DayReset & " AND Month(MSales.Tanggal)=" & MonthReset & " AND Year(MSales.Tanggal)=" & YearReset & " AND MSales.IDUser=" & IDUser & " AND MSales.Shift= " & NamaShiftReset & ") AS Retur, " & vbCrLf & _
'               " (SELECT SUM(MKasKeluar.Jumlah) AS KasKeluar FROM MKasKeluar WHERE Day(MKasKeluar.Tanggal)=" & DayReset & " AND Month(MKasKeluar.Tanggal)=" & MonthReset & " AND Year(MKasKeluar.Tanggal)=" & YearReset & " AND MKasKeluar.Shift= " & NamaShiftReset & ") AS KasKeluar " & vbCrLf & _
'               " FROM MSales  " & vbCrLf & _
'               " WHERE Msales.Subtotal<>0 and Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND SHIFT=" & NamaShiftReset & _
'               " GROUP BY IDUser, Shift"
  strqry = "SELECT MSales.IDUser, SUM(Subtotal) AS Subtotal1, SUM(DiscIntern) AS DiscNota1, SUM(Pembulatan) AS Pembulatan1, SUM(Subtotal-DiscIntern-Pembulatan) AS Total1, SUM(Voucher) AS Voucher1," & vbCrLf & _
               " SUM(HargaTotal-Bank-Voucher) AS Tunai1, " & vbCrLf & _
               " SUM(iif(IDBANK=1,Bank,0)) AS Debet, " & vbCrLf & _
               " SUM(iif(IDBank=2,MSales.BANK-(MSales.HargaTotal-(MSales.SubTotal-MSales.DiscIntern-MSales.Pembulatan)),0)) AS Kredit, " & vbCrLf & _
               " SUM(MSales.HargaTotal-(MSales.SubTotal-MSales.DiscIntern-MSales.Pembulatan)) AS Charge1, " & vbCrLf & _
               " COUNT(NoID) AS JumNota, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=5,BANK,0)) AS DebetMandiri, " & _
               " Sum(iif(IDJenisKartu=5,1,0)) AS CountDebetMandiri, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=3,BANK,0)) AS DebetBCA, " & _
               " Sum(iif(IDJenisKartu=3,1,0)) AS CountDebetBCA, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=6,BANK,0)) AS DebetLain, " & _
               " SUM(iif(IDJenisKartu=6,1,0)) AS CountDebetLain, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=1,BANK,0)) AS CreditVisa, " & _
               " SUM(iif(IDJenisKartu=1,1,0)) AS CountCreditVisa, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=2,BANK,0)) AS CreditMaster, " & _
               " SUM(iif(IDJenisKartu=2,1,0)) AS CountCreditMaster, " & vbCrLf & _
               " SUM(iif(IDJenisKartu=4,BANK,0))  AS CreditBCA, SUM(iif(IDJenisKartu=4,1,0)) AS CountCreditBCA, " & _
               " SUM(iif(IDJenisKartu=7,BANK,0))  AS CreditLain, SUM(iif(IDJenisKartu=7,1,0))  AS CountCreditLain " & vbCrLf & _
             " FROM MSales  " & vbCrLf & _
               " WHERE Msales.Subtotal<>0 and Day(Tanggal)=" & DayReset & " AND Month(Tanggal)=" & MonthReset & " AND Year(Tanggal)=" & YearReset & " AND SHIFT=" & NamaShiftReset & _
              " GROUP BY IDUser "
      Set rs = dbs.OpenRecordset(strqry)

      strqry = "SELECT MSales.IDUser,SUM(MSalesD.Jumlah) as JumlahRetur " & _
              " FROM MSales INNER JOIN MSalesD ON MSalesD.IDSales=MSales.NoID WHERE UCASE(MSalesD.Transaksi)='RTN' " & _
              "AND Day(MSales.Tanggal)=" & DayReset & " AND Month(MSales.Tanggal)=" & MonthReset & " AND Year(MSales.Tanggal)=" & YearReset & " AND SHIFT=" & NamaShiftReset & _
              " Group by MSales.IDUser"
      Set rsRetur = dbs.OpenRecordset(strqry)

      strqry = "SELECT MKasKeluar.IDKasir,SUM(MKasKeluar.Jumlah) AS KasKeluar " & _
            " FROM MKasKeluar " & _
            " WHERE Day(MKasKeluar.Tanggal)=" & DayReset & " AND Month(MKasKeluar.Tanggal)=" & MonthReset & " AND Year(MKasKeluar.Tanggal)=" & YearReset & " AND SHIFT=" & NamaShiftReset & _
            " Group by MKasKeluar.IDKasir"
      Set RsKasKeluar = dbs.OpenRecordset(strqry)

      If Not (rs.EOF And rs.BOF) Then
        rs.MoveFirst
        Do While Not rs.EOF
          If Not (RsReset.EOF And RsReset.BOF) Then
                  RsReset.MoveFirst
          End If
          RsReset.FindFirst "IDKasir=" & NullToNol(rs!IDUser) & " AND Shift=" & NamaShiftReset
          If Not RsReset.NoMatch Then
            RsReset.Edit
          Else
            NoIDReset = GetNewID("MReset")
            cariKodeNama KodeKasir_, NamaKasir_, rs!IDUser
            RsReset.AddNew
            RsReset!NoID = NoIDReset
            RsReset!Modal = 0
            RsReset!IDKasir = rs!IDUser
            RsReset!KodeKasir = KodeKasir_
            RsReset!NamaKasir = NamaKasir_
            RsReset!TANGGAL = CDate(MonthReset & "/" & DayReset & "/" & YearReset)
          End If
            If Text1.Text = 2 Then
              If Not NullToBool(RsReset!ResetZ) Then
                'RsReset!ResetZ = False 'True 'Pershift per kasir dianggap reset X semua
                RsReset!ResetX = True
                RsReset!ResetZ = False
                RsReset!SubTotal = NullToNol(rs!SubTotal1)
                RsReset!DiscMember = NullToNol(rs!DiscNota1)
                RsReset!Pembulatan = NullToNol(rs!Pembulatan1)
                RsReset!Total = NullToNol(rs!Total1)
                ...
                If Not (rsRetur.EOF And rsRetur.BOF) Then
                    rsRetur.FindFirst "IDuser=" & NullToNol(rs!IDUser)
                    If Not rsRetur.NoMatch Then
                        RsReset!Retur = NullToNol(rsRetur!JumlahRetur)
                    Else
                        RsReset!Retur = 0
                    End If
                Else
              ...
              If Not (RsKasKeluar.EOF And RsKasKeluar.BOF) Then
                    RsKasKeluar.FindFirst "IDKasir=" & NullToNol(rs!IDUser)
                    If Not RsKasKeluar.NoMatch Then
                        RsReset!JumlahKasKeluar = NullToNol(RsKasKeluar!KasKeluar)
                    Else
                        RsReset!JumlahKasKeluar = 0
                    End If
                Else
                    RsReset!JumlahKasKeluar = 0
                End If
                RsReset!IDPengawasZ = IDPengawas_
                RsReset!KodePengawasZ = KodePengawas_
                RsReset!NamaPengawasZ = NamaPengawas_
                RsReset!Shift = NamaShiftReset
                RsReset.Update
                RsReset.Bookmark = RsReset.LastModified
                PrintResetHartani NullToNol(RsReset!NoID)
            ...
            Else
              ...
              If Not (RsKasKeluar.EOF And RsKasKeluar.BOF) Then
                ...
                Else
                    RsReset!JumlahKasKeluar = 0
                End If
                RsReset!IDPengawasX = IDPengawas_
                RsReset!KodePengawasX = KodePengawas_
                RsReset!NamaPengawasX = NamaPengawas_
                RsReset!Shift = NamaShiftReset
                RsReset.Update
                RsReset.Bookmark = RsReset.LastModified
                PrintResetHartani NullToNol(RsReset!NoID)
      ...
      If Text1.Text = 2 Then
          If Not NullToBool(RsReset!ResetZ) Then
            ...
            RsReset!Charge = NullToNol(rs!Charge1)
            RsReset!Retur = NullToNol(rs!Retur)
            RsReset!JumlahKasKeluar = NullToNol(rs!KasKeluar)
        ...
        Else
          If Not NullToBool(RsReset!ResetZ) Then
            ...
            If Not (rsRetur.EOF And rsRetur.BOF) Then
                    rsRetur.FindFirst "IDuser=" & NullToNol(rs!IDUser)
                    If Not rsRetur.NoMatch Then
                        RsReset!Retur = NullToNol(rsRetur!Retur)
                    Else
                        RsReset!Retur = 0
                    End If
                Else
