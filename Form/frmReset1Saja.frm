...
'      rstTabel.MoveFirst
      ...
      For i = 0 To rstTabel.RecordCount - 1
        'CekServer
        SQL = "SELECT * FROM MJual WHERE NoIDPOS=" & NullToNol(rstTabel!NoID) & " AND IDPOS=" & CInt(IDPOSDef) & " AND Year(TANGGAL) = " & Right(Text2.Text, 4) & " And Month(TANGGAL) = " & Mid(Text2.Text, 3, 2) & " And Day(TANGGAL) = " & Left(Text2.Text, 2) & " And Shift = " & Text1.Text
...
''
''        SaldoKSBNotaIni = (BelanjaKSB - Voucher) - ((BelanjaKSB - Voucher + penambahanPointSebelumnya) \ 100000) * 100000
''
...
''                FixKoma(BelanjaKSB - Voucher) & "," & (BelanjaKSB - Voucher + penambahanPointSebelumnya) \ 100000 & "," & IIf(NamaMesin = "12" Or NamaMesin = "13", 1, 0) & "," & FixKoma(penambahanPointSebelumnya) & "," & FixKoma(SaldoKSBNotaIni) & ")"
'    End If
'   End If
'End Sub
...
On Error GoTo Trace
  ...
  Set rst = dbsr.OpenRecordset("SELECT SUM(MSales.HargaTotal-MSales.Pembulatan-MSales.DiscNota) as Total From MSales WHERE MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(1) = "FakturCash=0"
  Else
    CrystalReport1.Formulas(1) = "FakturCash=" & rst!Total
  End If
  
  Set rst = dbsr.OpenRecordset("SELECT SUM(MSales.UangMuka-(MSales.HargaTotal-MSales.Pembulatan-MSales.DiscNota)) as Total From MSales WHERE MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(2) = "FakturHutang=0"
  Else
    CrystalReport1.Formulas(2) = "FakturHutang=" & rst!Total
  End If

  Set rst = dbsr.OpenRecordset("SELECT COUNT(MSales.NoID) as Total From MSales WHERE HargaTotal<>0 AND MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(3) = "JumlahNota=0"
  Else
    CrystalReport1.Formulas(3) = "JumlahNota=" & rst!Total
  End If

  Set rst = dbsr.OpenRecordset("SELECT SUM(MSales.Pembulatan) as Total From MSales WHERE MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(4) = "TotalRounding=0"
  Else
    CrystalReport1.Formulas(4) = "TotalRounding=" & rst!Total
  End If

  Set rst = dbsr.OpenRecordset("SELECT SUM(MSales.HargaTotal-MSales.Pembulatan-MSales.DiscNota) as Total From MSales WHERE MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(5) = "TotalOmset=0"
  Else
    CrystalReport1.Formulas(5) = "TotalOmset=" & rst!Total
  End If

  Set rst = dbsr.OpenRecordset("SELECT SUM(MSales.HargaTotal-MSales.Pembulatan-MSales.DiscNota) as Total From MSales WHERE MONTH(Tanggal)=" & Mid(Text2.Text, 3, 2) & " AND YEAR(Tanggal)=" & Right(Text2.Text, 4) & " AND DAY(Tanggal)=" & Left(Text2.Text, 2))
  If rst.BOF And rst.EOF Then
    CrystalReport1.Formulas(6) = "TotalUangCash=0"
  Else
    CrystalReport1.Formulas(6) = "TotalUangCash=" & rst!Total
  End If

'  CrystalReport1.Formulas(7) = "NamaMesinReset='" & Format(Text3.Text, "00") & "'"
  CrystalReport1.Formulas(7) = "NamaPerusahaan='" & Trim(NamaToko) & "'"
  CrystalReport1.Formulas(8) = "AlamatPerusahaan='" & Trim(Replace(Judulstruk, vbCrLf, "'+ Chr(13) +'")) & "'"
